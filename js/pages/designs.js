...
registerPage('designs', async (container) => {
  container.innerHTML = `
    <div class="page-header">
      <div><div class="page-title">Design planner</div></div>
      <button class="btn btn-primary btn-sm" onclick="openAddDesign()">+ New design</button>
    </div>
  ...
  try {
    ...
    const cols = DESIGN_STAGES.map(stage => {
      ...
      const cards = items.length
        ...
        : `<div style="font-size:11px;color:var(--text3);padding:8px 4px">Nothing here</div>`;
      return `
        <div>
  ...
  return `<div class="pipeline-item" onclick="openDesignDetail('${d.id}')">
    ...
    <div style="display:flex;gap:6px;margin-top:8px;justify-content:flex-end">
      ${DESIGN_STAGES.findIndex(s=>s.key===d.stage) < DESIGN_STAGES.length-1
        ? `<button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();advanceDesign('${d.id}','${d.stage}')">Advance →</button>`
        : ''}
      <button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();openEditDesign('${d.id}')">Edit</button>
    </div>
  </div>`;
}
  ...
  openModal(d.name,
    `<div style="display:grid;gap:16px">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div class="stat-card"><div class="stat-label">Stage</div><div class="stat-value"><span class="badge badge-${d.stage}">${d.stage}</span></div></div>
        <div class="stat-card"><div class="stat-label">Category</div><div class="stat-value" style="font-size:14px">${d.category||'—'}</div></div>
      </div>
      <div>${stepsHTML}</div>
      ${d.printer ? `<div style="font-size:12px;color:var(--text2)"><strong style="color:var(--text)">Printer:</strong> ${d.printer}</div>` : ''}
      ${d.costEstimate ? `<div style="font-size:12px;color:var(--text2)"><strong style="color:var(--text)">Est. cost:</strong> ${fmt(d.costEstimate)} per unit</div>` : ''}
      ${d.minOrder ? `<div style="font-size:12px;color:var(--text2)"><strong style="color:var(--text)">Min order:</strong> ${d.minOrder} units</div>` : ''}
      ${d.notes ? `<div style="font-size:12px;color:var(--text2);border-top:1px solid var(--border);padding-top:12px">${d.notes}</div>` : ''}
    </div>`,
    `<button class="btn btn-ghost" onclick="closeModal()">Close</button>
     <button class="btn btn-ghost" onclick="closeModal();openEditDesign('${id}')">Edit</button>
     ${stageIdx < DESIGN_STAGES.length-1
       ? `<button class="btn btn-primary" onclick="closeModal();advanceDesign('${id}','${d.stage}')">Advance to ${DESIGN_STAGES[stageIdx+1]?.label}</button>`
       : ''}`
  );
}

function openAddDesign() {
  openModal('New design', buildDesignForm(null),
    `<button class="btn btn-ghost" onclick="closeModal()">Cancel</button>
     <button class="btn btn-primary" onclick="saveDesign(null)">Add design</button>`
  );
}

async function openEditDesign(id) {
  const d = await fsGet('merch_designs', id);
  openModal('Edit design', buildDesignForm(d),
    `<button class="btn btn-ghost" onclick="closeModal()">Cancel</button>
     <button class="btn btn-danger" onclick="closeModal();deleteDesign('${id}','${d.name}')">Delete</button>
...
function buildDesignForm(d) {
  return `
    <div class="field"><label>Design name</label><input id="df-name" type="text" value="${d?.name||''}"/></div>
    <div class="field-row">
      <div class="field"><label>Category</label>
        <select id="df-cat">
          <option value="clothing" ${(d?.category||'clothing')==='clothing'?'selected':''}>Clothing</option>
          <option value="records"  ${d?.category==='records'?'selected':''}>Records</option>
    ...
    <div class="field-row">
      <div class="field"><label>Printer</label><input id="df-printer" type="text" value="${d?.printer||''}" placeholder="e.g. Spreadshirt, local shop"/></div>
      <div class="field"><label>Min order qty</label><input id="df-min" type="number" value="${d?.minOrder||''}"/></div>
    </div>
    <div class="field-row">
      <div class="field"><label>Est. cost / unit (kr)</label><input id="df-cost" type="number" value="${d?.costEstimate||''}"/></div>
      <div class="field"><label>Target price (kr)</label><input id="df-price" type="number" value="${d?.targetPrice||''}"/></div>
    </div>
    <div class="field"><label>Notes / references</label><textarea id="df-notes">${d?.notes||''}</textarea></div>
  `;
}
...
async function advanceDesign(id, currentStage) {
  const idx  = DESIGN_STAGES.findIndex(s => s.key === currentStage);
  const next = DESIGN_STAGES[idx + 1];
  if (!next) return;
  const d = await fsGet('merch_designs', id);
  await fsSet('merch_designs', id, { ...d, stage: next.key, updatedAt: now() });
  showToast(`Moved to ${next.label}`);
  await renderDesigns();
}

async function deleteDesign(id, name) {
  confirmAction(`Delete design "${name}"?`, async () => {
