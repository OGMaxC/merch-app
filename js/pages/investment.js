...
async function renderInvestering() {
  ...
  try {
    ...
    const totalÅtervunnet = sales.reduce((s, t) => s + (t.amount || 0), 0);
    const pravit        = Math.max(0, totalÅtervunnet - totalInvested);
    const stillOwed     = Math.max(0, totalInvested - totalÅtervunnet);

    /* per-person breakdown */
    const personData = {};
    for (const p av PERSONS) {
    ...
    for (const p av PERSONS) {
      ...
      personData[p].recouped = Math.min(personData[p].invested, totalÅtervunnet * share);
      personData[p].skyldig     = Math.max(0, personData[p].invested - personData[p].recouped);
    }

    const pct = totalInvested > 0 ? Math.min(100, Math.round(totalÅtervunnet / totalInvested * 100)) : 0;

    el.innerHTML = `
      <div class="stat-grid">
        <div class="stat-card"><div class="stat-label">Totalt investerat</div><div class="stat-value">${fmt(totalInvested)}</div></div>
        <div class="stat-card"><div class="stat-label">Återvunnet</div><div class="stat-value gold">${fmt(totalÅtervunnet)}</div></div>
        <div class="stat-card"><div class="stat-label">Kvar att återvinna</div><div class="stat-value amber">${fmt(stillOwed)}</div></div>
        <div class="stat-card"><div class="stat-label">Vinst (efter återvinning)</div><div class="stat-value green">${fmt(pravit)}</div></div>
      </div>
            ...
            ${PERSONS.map(p => {
              ...
              return `<div class="card-body" style="border-bottom:1px solid var(--bg3)">
                <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:4px">
                  ...
                  <span style="font-size:12px;color:${d.skyldig>0?'var(--amber)':'var(--green)'}">
                    ${d.skyldig > 0 ? `${fmt(d.skyldig)} skyldig` : 'Återvunnet'}
                  </span>
                </div>
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div style="font-size:11px;color:var(--text3);margin-top:8px">
                ${fmt(totalÅtervunnet)} av ${fmt(totalInvested)} återvunna
              </div>
              ${pravit > 0 ? `<div style="margin-top:12px;padding:10px;background:var(--green-bg);border-radius:6px;font-size:12px;color:var(--green)">
                Investerings fully recouped. Pravit to split: ${fmt(pravit)} (${fmt(pravit/3)} vardera)
              </div>` : ''}
            </div>
          </div>
        </div>
...
async function deleteTransaction(id) {
  confirmAction('Ta bort this transaction?', async () => {
    await fsTa bort('merch_transactions', id);
